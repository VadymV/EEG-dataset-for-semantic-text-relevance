...
class LSTM(nn.Module):
    ...
    Args:
        input_dim (int): The number of electrodes, i.e., :math:`C` in the paper. (default: :obj:`32`)
    ...
    def __init__(self,
        ...
        self.gru_layer = nn.LSTM(input_size=input_dim,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        r'''
        Args:
            x (torch.Tensor): EEG signal representation, the ideal input shape is :obj:`[n, time, features]`. Here, :obj:`n` corresponds to the batch size, :obj:`32` corresponds to :obj:`num_electrodes`, and :obj:`128` corresponds to the number of data points included in the input EEG chunk.
        ...
        # x = self.out(r_out[:, -1, :]).reshape((x.shape[0],))  # choose r_out at the last time step
        x = self.out(h[-1, :, :])
